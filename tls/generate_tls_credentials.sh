...
#
# Note: when running on Windows Git Bash set MSYS_NO_PATHCONV to 1
# e.g.,  export MSYS_NO_PATHCONV=1
#
...
if [ "$#" -ne 4 ]; then
	cat << EOF
$scriptName <organisation> <domain> <sub-domain> <secret-name>
...
kubectl -n istio-system create secret tls $secretName \
	--key=$serverKeyFile --cert=$serverCertFile --dry-run=client -o yaml > $secretName.yaml
